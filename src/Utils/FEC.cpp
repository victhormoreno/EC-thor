...
FEC::FEC(const Params::ObjectDetector &params) {
...
std::vector<pcl::PointIndices> FEC::fastClustering(pcl::PointCloud<pcl::PointXYZI>::Ptr cloud, 
                                         float epsilon ,int min_component_size, int max_n){
    ...
    for (i = 0; i < cloud_size; i++)
    {
        ...
        if (marked_indices[i] == 0) // reset to initial value if this point has not been manipulated
        {
            ...
            cloud_kdtreeflann.radiusSearch(cloud->points[i], epsilon, pointIdx, pointquaredDistance);
            /**
